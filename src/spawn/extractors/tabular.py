...
class TabularMetadataExtractor(MetadataExtractor):
    ...
    def _extract_from_delimited(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8", errors="replace") as f:
                ...
                # Sample rows
                sample_rows = []
                for _ in range(min(self.max_rows_to_sample, 10)):  # Just read a few rows for basic info
                    line = f.readline()
                    if not line:
                        break
            ...
            if sample_rows:
                metadata["sample_statistics"] = self._calculate_statistics(columns, sample_rows)
    ...
    def _extract_from_spreadsheet(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata["note"] = "Full spreadsheet metadata extraction requires additional libraries"
    ...
    def _detect_column_types(self, columns: List[str], sample_rows: List[List[str]]) -> Dict[str, str]:
        """
    ...
    def _detect_value_type(self, values: List[Any]) -> str:
        ...
        date_count = 0
        for v in non_empty:
            if isinstance(v, str) and any(re.match(pattern, v) for pattern in date_patterns):
                date_count += 1
    ...
    def _calculate_statistics(self, columns: List[str], sample_rows: List[List[str]]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for i, col in enumerate(columns):
            ...
            if numeric_values:
                ...
                if len(sorted_values) % 2 == 0:
                    col_stats["median"] = (sorted_values[mid-1] + sorted_values[mid]) / 2
                else:
