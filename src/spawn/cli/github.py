...
from spawn.utils.github import GitHubClient, configure_static_json, fork_template_portal, configure_pages_and_actions
...
@click.option(
    "--token", help="GitHub personal access token (overrides config and environment)"
)
...
@click.option(
    "--pages-branch",
    default="gh-pages",
    help="Branch to deploy GitHub Pages from (default: gh-pages)",
)  # TODO: Ask ryan about this
@click.option(
    "--pages-path",
    default="/",
    help="Path in the repo to deploy for GitHub Pages (default: /)",
)
def fork_portal(
    name: str,
    ...
    try:
        ...
        # Now determine repo_owner safely
        repo_owner = (
            organization
            or username
            or result["repository"].get("owner", {}).get("login")
        )
        # Configure GitHub Pages and Actions
        try:
            configure_pages_and_actions(
            ...
            print(
                f"Configured GitHub Pages and Actions for {repo_owner}/{repo_name} (branch: {pages_branch}, path: {pages_path})"
            )
        except Exception as e:
            logger.error(f"Error configuring GitHub Pages and Actions: {e}")
            print(
                f"Warning: Repository forked, but failed to configure Pages/Actions: {e}"
            )
    except Exception as e:
...
@click.option(
    "--push/--no-push", default=False, help="Whether to push the changes to GitHub"
)
@click.option(
    "--repo-owner", help="Owner of the repository (required if --push is used)"
)
...
@click.option(
    "--commit-message", default="Configure portal", help="Commit message for the push"
)
